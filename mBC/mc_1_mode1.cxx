...
for (int nmode=1; nmode<2; nmode++) {
    ...
    TFile *f = TFile::Open(Form("./root/event1_datahist%d.root",nmode));
   ...
   if(nmode==0)    RooRealVar  mean1( "mean1" , "", -1e-3,1e-3);
   if(nmode==0)    RooRealVar  mean2( "mean2" , "", -2e-3,2e-3);
   if(nmode==0)    RooRealVar  sigma1("sigma1", "", 0.,2e-3);
   if(nmode==0)    RooRealVar  sigma2("sigma2", "", 0.,3e-3);

   if(nmode==1)    RooRealVar  mean1( "mean1" , "", -2e-3,2e-3);  
   if(nmode==1)    RooRealVar  mean2( "mean2" , "", -3e-3,3e-3);
   if(nmode==1)    RooRealVar  sigma1("sigma1", "", 0.,3e-3);
   if(nmode==1)    RooRealVar  sigma2("sigma2", "", 0.,2e-3);
   
   if(nmode==3)    RooRealVar  mean1( "mean1" , "", -1e-3,1e-3);
   if(nmode==3)    RooRealVar  mean2( "mean2" , "", -1e-2,1e-2);
   if(nmode==3)    RooRealVar  sigma1("sigma1", "", 0.,2e-3);
   if(nmode==3)    RooRealVar  sigma2("sigma2", "", 0.,1e-3);
                                                             
    RooGaussian gauss1("gauss1", "", mBC, mean1, sigma1 );
    RooGaussian gauss2("gauss2", "", mBC, mean2, sigma2 );
    RooRealVar  frac1("frac1","", 0.5,0.0, 1.0) ;
    ...
    if(nmode!=3) RooRealVar arg2  ("arg2", "", 0.3, 10.);   
    RooArgusBG bkg("bkg","",mBC,endpt,kappa,arg2);
    
    double sum_data = hdata.sumEntries();   
    if(nmode==0)    RooRealVar Nsig("Nsig","", 0.9*sum_data, 0.0,sum_data);     //mode0(0.8,0)
    if(nmode==0)    RooRealVar Nbkg("Nbkg","", 0.01*sum_data,0.0,sum_data); //mode0(0.1,0,0.3)
    if(nmode==1)    RooRealVar Nsig("Nsig","", 0.6*sum_data, 0.0, sum_data);
    if(nmode==1)    RooRealVar Nbkg("Nbkg","", 0.4*sum_data, 0.0, sum_data);
    if(nmode==3)    RooRealVar Nsig("Nsig","", 0.7*sum_data, 0.0, sum_data);
    if(nmode==3)    RooRealVar Nbkg("Nbkg","", 0.3*sum_data, 0.0, sum_data);
...
*/
    while ((edm>1||status!=0||covQual!=3/*||chi2>3*/)&&times<1000){
     Double_t m_mean1  = r1.Uniform(mean1.getMin()+2e-4, mean1.getMax()-2e-4); 
     Double_t m_mean2  = r1.Uniform(mean2.getMin()+3e-4, mean2.getMax()-3e-4);
     Double_t m_sigma1 = r1.Uniform(sigma1.getMin()+3e-3,sigma1.getMax()-3e-3);
     Double_t m_sigma2 = r1.Uniform(sigma2.getMin()+2e-3,sigma2.getMax()-2e-3);
     Double_t m_frac1  = r1.Uniform(frac1.getMin()+1e-2,frac1.getMax()-1e-2);     
     Double_t m_kappa  = r1.Uniform(kappa.getMin()+1,kappa.getMax()-1);
     Double_t m_arg2   = r1.Uniform(arg2.getMin()+0.05,arg2.getMax()-0.05);
   ...
   */      
    ...
    */
    ...
    double N_dcs0 = 2278.4,   N_dcs1  = 2465.1,   N_dcs3  = 2352.1,  N_107 = 1113.0;
    double N_prod0 = 902967., N_prod1 = 3230730., N_prod3 = 1944343.;
    if (nmode==0) double eff = (N_fit-N_dcs0)/N_prod0;
    if (nmode==1) double eff = (N_fit-N_dcs1)/N_prod1;
    if (nmode==3) double eff = (N_fit-N_dcs3-N_107)/N_prod3;      

    /************************************************end********************************************************/
    ofstream fit_data;
    fit_data.open(Form("./figure/mc_1_%d.dat",nmode));
    ...
    */TLatex lt1;
     ...
     if (nmode==0)         lt1.DrawLatex(0.22, 0.81,  "#bar{D}^{0}#rightarrowK^{+}#pi^{-}");//mode0
     else if (nmode==1)    lt1.DrawLatex(0.22, 0.81,  "#bar{D}^{0}#rightarrowK^{+}#pi^{-}#pi^{0}");//mode1
     else                  lt1.DrawLatex(0.22, 0.81,  "#bar{D}^{0}#rightarrowK^{+}#pi^{-}#pi^{+}#pi^{-}");//mode3
    //TArrow *ar1 = new TArrow(1.858,0.4*hshape->GetMaximum(),1.858,0.1*hshape->GetMaximum(),0.01,"|>");
    //TArrow *ar2 = new TArrow(1.874,0.4*hshape->GetMaximum(),1.874,0.1*hshape->GetMaximum(),0.01,"|>");
    //mode0
    if(nmode==0) TArrow *ar1 = new TArrow(1.858, 0.5*h1->GetMaximum(),1.858,0.1*h1->GetMaximum() ,0.01,"|>");
    if(nmode==0) TArrow *ar2 = new TArrow(1.874, 0.5*h1->GetMaximum(),1.874,0.1*h1->GetMaximum() ,0.01,"|>");
    ...
    if(!(nmode==0)) TArrow *ar2 = new TArrow(1.874, 0.5*h1->GetMaximum(),1.874,0.1*h1->GetMaximum() ,0.01,"|>");
     ...
     cvs->Print(Form("figure/mc_1_%d.eps",nmode),"recreate");    
     cout<<"chi2 "<<chi2<<endl;
     cout<<"eff "<<eff<<endl;
     }
}
