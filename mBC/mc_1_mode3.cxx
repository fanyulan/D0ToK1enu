...
for (int nmode=3; nmode<4; nmode++) {
    ...
    TFile *f = TFile::Open("et1_dh3_data_all.root");
   ...
   if(nmode==3)    RooRealVar  sigma2("sigma2", "", 0.,1e-2);
    ...
    if(nmode==3)    RooRealVar Nsig("Nsig","", 0.9*sum_data, 0.0, sum_data);
    if(nmode==3)    RooRealVar Nbkg("Nbkg","", 0.1*sum_data, 0.0, sum_data);
...
*/
    while ((edm>1||status!=0||covQual!=3/*||chi2>3*/)&&times<1000){
     Double_t m_mean1  = r1.Uniform(-2e-3,2e-3); 
     Double_t m_mean2  = r1.Uniform(-2e-3,2e-3);
     Double_t m_sigma1 = r1.Uniform(0,2e-3);
     Double_t m_sigma2 = r1.Uniform(0,1e-2);
  ...
  //plot on 
    ...
    rp1->SetMarkerSize(0.5);
     ...
     lt1.DrawLatex(0.22, 0.76,  "#chi^{2}_{ndof} = 2.0");
    ...
    double N_dcs3  = 2*N*2.65e-4*0.382, N_107 = 2*N*2.17e-3*0.692*0.0319; //N_107 = 2*N_prod3*2.17e-3*0.692*0.0319;
    double eff = (N_fit-N_dcs3-N_107)/N_prod3;

    /************************************************end********************************************************/
    ofstream fit_data;
    fit_data.open(Form("./figure/mc_1_%d_data.dat",nmode), ios::app);
    ...
    */
     ...
     cvs->Print(Form("figure/mc_1_%d_data.eps",nmode),"recreate");    
     cout<<"chi2 "<<chi2<<endl;
     cout<<"eff "<<eff<<endl;
     }
}
