...
void eff()
{
  ...
  TCut cut_mc3  = "(mcmodea==666||mcmodea==222) && (mcmodeb==-103 || mcmodeb==-108 || mcmodeb==-1032) && mcmode1==103";
  TCut cut_mc31 = "(mcmodea==103 || mcmodea==108 || mcmodea==1032) && (mcmodeb==-666||mcmodeb==-222) && mcmode2==-103";
  ...
  //cut
  TChain t0("tagD");
  t0.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sigMC/update/PDG2020/DDecay_root/red_truth/truth*.root"); 
  TTree *T0_tagD = t0.CopyTree(cut_sig&&cuta&&cutb&&cutc&&cut_K1&&cut0);//mode0
  TTree *T1_tagD = t0.CopyTree(cut_sig&&cuta&&cutb&&cutc&&cut_K1&&cut1);
  TTree *T3_tagD = t0.CopyTree(cut_sig&&cuta&&cutb&&cutc&&cut_K1&&cut3);
  TChain t1("truth");
  TChain t2("truth");
  t1.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sigMC/update/PDG2020/DDecay_root/d0/*.root");
  t2.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sigMC/update/PDG2020/DDecay_root/d0bar/*.root");
  t1.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sigMC/update/PDG2020/KsKpi/d0/*.root");
  t2.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sigMC/update/PDG2020/KsKpi/d0bar/*.root");
...
*/double ST0_eff = 0.66089, ST1_eff = 0.349938, ST3_eff = 0.4105;
  ...
  double SL3_eff_err=sqrt(ST3_eff*ST3_eff*DT3_eff_err*DT3_eff_err + DT3_eff*DT3_eff*ST3_eff_err*ST3_eff_err)/(ST3_eff*ST1_eff);

  double N_st0 = 542153., N_st1 = 1084880., N_st3 =740957.,  N_ST_total =2367990.;//fanyl
  ...
  //write
  ofstream out;
  out.open("./dat/SL.dat");//mode0
  out<<"produced number in sigMC: "<< N0total+N01total<<"\t"<<N1total+N11total<<"\t"<<N3total+N31total<<"\n";
  out<<"observed number in sigMC: "<< N0obs<<"\t"<<N1obs<<"\t"<<N3obs<<"\n";
  out<<"total obs: "<< N0obs+N1obs+N3obs<<"\n";
  out<<"DT efficiency : "<<"\n";
  out<<DT0_eff<<"\t"<<DT0_eff_err<<"\n";
  out<<DT1_eff<<"\t"<<DT1_eff_err<<"\n";
  out<<DT3_eff<<"\t"<<DT3_eff_err<<"\n";
  out<<"SL efficiency : "<<"\n";
  out<<SL0_eff<<"\t"<<SL0_eff_err<<"\n";  
  out<<SL1_eff<<"\t"<<SL1_eff_err<<"\n";
  out<<SL3_eff<<"\t"<<SL3_eff_err<<"\n";  
  out<<eff_ave<<"\t"<<eff_ave_err;

  ofstream read;
  read.open("./dat/SL.txt");
  read<<SL0_eff<<"\n";
  read<<SL1_eff<<"\n";
  read<<SL3_eff<<"\n";
  read<<eff_ave;
  ofstream err("./dat/err.txt");
  err<<SL0_eff_err<<"\n";
  err<<SL1_eff_err<<"\n";
  err<<SL3_eff_err<<"\n";
  err<<eff_ave_err;
