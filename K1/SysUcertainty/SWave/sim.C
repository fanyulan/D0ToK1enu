...
TCut normal  = "isbest==1&&type==1&&mBC<1.874&&mBC>1.858&&mD_EasPi<1.81&&mD_EasPiwPi0<1.4&&costh_lep_pi<0.94&&costh_miss_neut<0.81&&((eop_E-0.42)>0.14*dedxchi_E)&&m_pipi>0.31&&abs(m_pipi-0.497611)>0.01&&abs(deltaE_pipzswp)>0.014&&abs(Umissfit)<0.1&&elecCharge!=0&&(elecCharge+kaonCharge)==0&&(pionCharge+pion2Charge)==0&&charm==kaonCharge";
...
TCut m3 = "mode==3&&deltaE>-0.031&&deltaE<0.028";
    ...
    double smooth_bkg  = 1.0;
    double smooth_k3pi = 1.0;
    double smooth_unre = 1.0;
    //mode0
    ...
    TChain* ch_bkg0_unre = new TChain("bkg");
    ch_bkg0_unre->Add("pdf/unresonance_bkg_0.root");
    RooDataSet *bkgshape0_unre = new RooDataSet("bkgshape0_unre", "bkgshape0_unre", ch_bkg0_unre, m_u);
    RooNDKeysPdf pbkg0_unre("pbkg0_unre","",m_u,*bkgshape0_unre, "am",smooth_unre,nSigma);
    //mode1
    ...
    TChain* ch_bkg1_unre = new TChain("bkg");
    ch_bkg1_unre->Add("pdf/unresonance_bkg_1.root");
    RooDataSet *bkgshape1_unre = new RooDataSet("bkgshape1_unre", "bkgshape1_unre", ch_bkg1_unre, m_u);
    RooNDKeysPdf pbkg1_unre("pbkg1_unre","",m_u,*bkgshape1_unre, "am",smooth_unre,nSigma);
    //mode3
    ...
    TChain* ch_bkg3_unre = new TChain("bkg");
    ch_bkg3_unre->Add("pdf/unresonance_bkg_2.root");
    RooDataSet *bkgshape3_unre = new RooDataSet("bkgshape3_unre", "bkgshape3_unre", ch_bkg3_unre, m_u);
    RooNDKeysPdf pbkg3_unre("pbkg3_unre","",m_u,*bkgshape3_unre, "am",smooth_unre,nSigma);
  double eff[4];
  for(int i=0;i<4;i++){
  ...
  ntag>>Ntag[0]>>Ntag[1]>>Ntag[2];
    ...
    RooRealVar nbkg0("nbkg0", "", 10,1,50); 
    RooRealVar nbkg1("nbkg1", "", 30,1,100);
    RooRealVar nbkg3("nbkg3", "", 20,1,100);
    
    double mis[3];
    for(int i=0;i<3;i++){
    ifstream nmis("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/script/sim_fit2D/misidentify/dat/misidentify103.dat");
    ...
    RooRealVar nbkg0_unre("nbkg0_unre", "", 4.11);//28.931 * 0.142 
    RooRealVar nbkg1_unre("nbkg1_unre", "", 7.81);//55.016 * 0.142
    RooRealVar nbkg3_unre("nbkg3_unre", "", 4.47);//31.509 * 0.142
    
    RooAddPdf allpdf0("allpdf0", "", RooArgList(psig0,pbkg0,pbkg0_k3pi, pbkg0_unre ), RooArgList(nsig0,nbkg0,nbkg0_k3pi,nbkg0_unre));
    RooAddPdf allpdf1("allpdf1", "", RooArgList(psig1,pbkg1,pbkg1_k3pi, pbkg1_unre ), RooArgList(nsig1,nbkg1,nbkg1_k3pi,nbkg1_unre));
    RooAddPdf allpdf3("allpdf3", "", RooArgList(psig3,pbkg3,pbkg3_k3pi, pbkg3_unre ), RooArgList(nsig3,nbkg3,nbkg3_k3pi,nbkg3_unre));
...
//    simPdf.plotOn(frame0, RooFit::Slice(sample,"mode0"), RooFit::ProjWData(sample, combData), RooFit::Components(RooArgSet(*psig0,*pomegasig)),RooFit::LineStyle(kDashed), RooFit::LineColor(kBlue));
    ...
    simPdf.plotOn(frame, RooFit::Slice(sample,"mode0"), RooFit::ProjWData(sample, combData), RooFit::Components(pbkg0_unre),RooFit::LineStyle(kDashed), RooFit::LineColor(kBlue-9));
//    simPdf.plotOn(frame0, RooFit::Slice(sample,"mode0"), RooFit::ProjWData(sample, combData), RooFit::Components(*pomegasig),RooFit::LineStyle(kDashed), RooFit::LineColor(kGreen+3));
    ...
    simPdf.plotOn(frame, RooFit::Slice(sample,"mode0"), RooFit::ProjWData(sample, combData), RooFit::Components(pbkg1_unre),RooFit::LineStyle(kDashed), RooFit::LineColor(kBlue-9));
   ...
   frame1->GetXaxis()->SetLabelSize(0.0);
    ...
    l0.DrawLatex(0.55, 0.78,  Form("nbkg = %.1f #pm %.1f",nbkg0.getVal()+nbkg0_k3pi.getVal()+nbkg0_unre.getVal(),n0_err));
    ...
    simPdf.plotOn(frame, RooFit::Slice(sample,"mode1"), RooFit::ProjWData(sample, combData), RooFit::Components(pbkg1_unre),RooFit::LineStyle(kDashed), RooFit::LineColor(kBlue-9));
    frame->GetYaxis()->SetRangeUser(0,24);
   ...
   TGaxis *axis1 = new TGaxis(-0.1,0,-0.1,24,0,24,510,"");
   axis1->SetName("axis1");
   axis1->SetLabelSize(0.1);
   axis1->SetLabelOffset(0.01);
   axis1->SetNdivisions(505);
   ...
   c1->cd();
    ...
    simPdf.plotOn(frame1, RooFit::Slice(sample,"mode1"), RooFit::ProjWData(sample, combData), RooFit::Components(pbkg1_unre),RooFit::LineStyle(kDashed), RooFit::LineColor(kBlue-9));
   ...
   frame1->GetXaxis()->SetLabelSize(0.0);
    ...
    l1.DrawLatex(0.55, 0.78,  Form("nbkg = %.1f #pm %.1f",nbkg1.getVal()+nbkg1_k3pi.getVal()+nbkg1_unre.getVal(),n1_err));
    ...
    simPdf.plotOn(frame, RooFit::Slice(sample,"mode3"), RooFit::ProjWData(sample, combData), RooFit::Components(pbkg3_unre),RooFit::LineStyle(kDashed), RooFit::LineColor(kBlue-9));

    frame->GetYaxis()->SetRangeUser(0,14);
   ...
   frame->GetXaxis()->SetLabelSize(0.0);
    ...
    TGaxis *axis1 = new TGaxis(-0.1,0,-0.1,14,0,14,510,"");
    axis1->SetName("axis1");
    axis1->SetLabelSize(0.1);
    axis1->SetLabelOffset(0.01);
    axis1->SetNdivisions(505);
   ...
   c1->cd();
    ...
    simPdf.plotOn(frame1, RooFit::Slice(sample,"mode3"), RooFit::ProjWData(sample, combData), RooFit::Components(pbkg3_unre),RooFit::LineStyle(kDashed), RooFit::LineColor(kBlue-9));
   ...
   axis2->Draw();
    ...
    l3.DrawLatex(0.55, 0.78,  Form("nbkg = %.1f #pm %.1f",nbkg3.getVal()+nbkg3_k3pi.getVal()+nbkg3_unre.getVal(),n3_err));
...
///////////////////////////////////////////////////////////////////modes///////////////////
    ...
    simPdf.plotOn(framex, RooFit::ProjWData(sample, combData), RooFit::Components(RooArgSet(pbkg0_unre,pbkg1_unre,pbkg3_unre)),RooFit::LineStyle(kDashed), RooFit::LineColor(kBlue-9));
    ...
    simPdf.plotOn(framey, RooFit::ProjWData(sample, combData), RooFit::Components(RooArgSet(pbkg0_unre,pbkg1_unre,pbkg3_unre)),RooFit::LineStyle(2), RooFit::LineColor(kBlue-9));
    framey->SetXTitle("M_{K#pi#pi} (GeV^{2})");
    framey->SetTitle("simultaneous fit");
    framey->Draw();

    RooRealVar nsig_sp_m0("nsig_sp_m0", "", nsig0.getVal());
    RooAddPdf allpdf_sp_m0("allpdf_sp_m0", "", RooArgList(psig0,pbkg0,pbkg0_k3pi,pbkg0_unre), RooArgList(nsig_sp_m0,nbkg0,nbkg0_k3pi,nbkg0_unre));
    RooRealVar nsig_sp_m1("nsig_sp_m1", "", nsig1.getVal());
    RooAddPdf allpdf_sp_m1("allpdf_sp_m1", "", RooArgList(psig1,pbkg1,pbkg1_k3pi,pbkg1_unre), RooArgList(nsig_sp_m1,nbkg1,nbkg1_k3pi,nbkg1_unre));
    RooRealVar nsig_sp_m3("nsig_sp_m3", "", nsig3.getVal());
    RooAddPdf allpdf_sp_m3("allpdf_sp_m3", "", RooArgList(psig3,pbkg3,pbkg3_k3pi,pbkg3_unre), RooArgList(nsig_sp_m3,nbkg3,nbkg3_k3pi,nbkg3_unre));
    //mode0
    RooStats::SPlot* splot = new RooStats::SPlot("splot","splot",*ds0,&allpdf_sp_m0,RooArgList(nsig_sp_m0,nbkg0,nbkg0_k3pi,nbkg0_unre));
    splot->Print();
    ...
    delete splot; 
    //mode1
    splot = new RooStats::SPlot("splot","splot",*ds1,&allpdf_sp_m1,RooArgList(nsig_sp_m1,nbkg1,nbkg1_k3pi,nbkg1_unre));
    splot->Print();
    delete dataw_s; 
    ...
    delete splot;
    //mode3
    splot = new RooStats::SPlot("splot","splot",*ds3,&allpdf_sp_m3,RooArgList(nsig_sp_m3,nbkg3,nbkg3_k3pi,nbkg3_unre));
    splot->Print();
    delete dataw_s; 
    ...
    out<<"mode0 bkg    : "<<nbkg0.getVal()+nbkg0_k3pi.getVal()+nbkg0_unre.getVal()<<"\t"<<n0_err<<"\n";
    out<<"mode1 bkg    : "<<nbkg1.getVal()+nbkg1_k3pi.getVal()+nbkg1_unre.getVal()<<"\t"<<n1_err<<"\n";
    out<<"mode3 bkg    : "<<nbkg3.getVal()+nbkg3_k3pi.getVal()+nbkg3_unre.getVal()<<"\t"<<n3_err<<"\n";
    out<<"total fit    : "<<Nsig0+Nsig1+Nsig3<<"+/-"<<Nsig1_err*(Nsig0+Nsig1+Nsig3)/Nsig1;
     ...
     ofstream out_1("figure/bf.txt", ios::app);
