...
TCut normal  = "isbest==1&&type==1&&mBC<1.874&&mBC>1.858&&mD_EasPi<1.81&&mD_EasPiwPi0<1.4&&costh_lep_pi<0.94&&costh_miss_neut<0.81&&((eop_E-0.32)>0.18*dedxchi_E)&&m_pipi>0.31&&abs(m_pipi-0.497611)>0.01&&abs(deltaE_pipzswp)>0.016&&abs(Umissfit)<0.1&&elecCharge!=0&&(elecCharge+kaonCharge)==0&&(pionCharge+pion2Charge)==0&&charm==kaonCharge";
...
TCut m3 = "mode==3&&deltaE>-0.031&&deltaE<0.028";
    ...
    double smooth_bkg = 0.9,  smooth_k3pi = 1.1;
   //mode0
    TChain* ch_sig0 = new TChain("sig");
    ch_sig0->Add("dat/sig0_plus.root");
    ...
    ch_bkg0->Add("dat/offical_bkg0_plus.root");
    ...
    ch_bkg0_k3pi->Add("dat/k3pi_bkg0_plus.root");
    ...
    ch_sig1->Add("dat/sig1_plus.root");
    ...
    ch_bkg1->Add("dat/offical_bkg1_plus.root");
    ...
    ch_bkg1_k3pi->Add("dat/k3pi_bkg1_plus.root");
    ...
    ch_sig3->Add("dat/sig2_plus.root");
    ...
    ch_bkg3->Add("dat/offical_bkg2_plus.root");
    ...
    ch_bkg3_k3pi->Add("dat/k3pi_bkg2_plus.root");
  ...
  MIS>>mis[0]>>mis[1]>>mis[2];
    ...
    RooRealVar nbkg1("nbkg1", "", 40,1,100); //nbkg.setConstant(1);
...
///////////////////////////////////////////////////////////////////modes///////////////////
    ...
    hmhadall->SetXTitle("M(K#pi#pi) [GeV]");
     ...
     c1->Print("figure/plus_modes.eps");
    cv2->Print("figure/plus.eps");
    ofstream out;
    out.open("figure/plus.dat");
    ...
    out<<"total fit    : "<<Nsig0+Nsig1+Nsig3<<"+/-"<<Nsig1_err*(Nsig0+Nsig1+Nsig3)/Nsig1;
     ...
     ofstream out_1("figure/bf_plus.txt", ios::app);
