...
void misidentify103()
{
  ...
  TCut cutb_minus    = "isbest==1&&type==1&&mBC<1.874&&mBC>1.858&&mD_EasPi<1.805&&mD_EasPiwPi0<1.4&&costh_lep_pi<0.94&&costh_miss_neut<0.81&&((eop_E-0.32)>0.18*dedxchi_E)&&m_pipi>0.31&&abs(m_pipi-0.497611)>0.01&&abs(deltaE_pipzswp)>0.012";
  TCut cutb_plus    = "isbest==1&&type==1&&mBC<1.874&&mBC>1.858&&mD_EasPi<1.815&&mD_EasPiwPi0<1.4&&costh_lep_pi<0.94&&costh_miss_neut<0.81&&((eop_E-0.32)>0.18*dedxchi_E)&&m_pipi>0.31&&abs(m_pipi-0.497611)>0.01&&abs(deltaE_pipzswp)>0.012";
  ...
  for(int m = 0; m<2; m++){
  if(m == 0){
  TTree *T0_tagD = t0.CopyTree(!(cut_sig&&cut_K1)&&cuta&&cutb_minus&&cutc&&cut0&&cut_k3pi);//mode0
  TTree *T1_tagD = t0.CopyTree(!(cut_sig&&cut_K1)&&cuta&&cutb_minus&&cutc&&cut1&&cut_k3pi);
  TTree *T3_tagD = t0.CopyTree(!(cut_sig&&cut_K1)&&cuta&&cutb_minus&&cutc&&cut3&&cut_k3pi);
  }
  if(m == 1){
  TTree *T0_tagD = t0.CopyTree(!(cut_sig&&cut_K1)&&cuta&&cutb_plus&&cutc&&cut0&&cut_k3pi);//mode0
  TTree *T1_tagD = t0.CopyTree(!(cut_sig&&cut_K1)&&cuta&&cutb_plus&&cutc&&cut1&&cut_k3pi);
  TTree *T3_tagD = t0.CopyTree(!(cut_sig&&cut_K1)&&cuta&&cutb_plus&&cutc&&cut3&&cut_k3pi);
  }
  TChain t1("truth");
  TChain t2("truth");
  ...
  ofstream out1;
  if(m == 0)out1.open("./dat/mis_minus.dat");
  if(m == 1)out1.open("./dat/mis_plus.dat");
  out1<<N0<<"\n";
  out1<<N1<<"\n";
  out1<<N3<<"\n";
