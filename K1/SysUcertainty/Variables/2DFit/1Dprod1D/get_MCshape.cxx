//backgroud analysis through DDmcmode,
...
void get_MCshape()
{
   ...
   TChain t("tagD");
   t.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sigMC/update/BELLE/fit2/red_truth/truth.root");//sig mc
   //t.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/new/inclusiveMC/umiss2fit/reduce_truth/root/truth*.root");
   //t.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sigMC/update/K3pi/red_truth/truth*.root");
   ...
   for( unsigned int i=0; i<t.GetEntries(); i++ ){
       ...
       bool cutb =fabs(Umissfit)<0.1&&mhad<1.68&&mhad>0.9;
       bool cut_k3pi    = (charm==1&&(mcmodeb==-103||mcmodeb==-1032||mcmodeb==-108))||(charm==-1&&(mcmodea==103||mcmodea==1032||mcmodea==108));

       bool sig_shape  = cuta&&cutb&&cutc&&cut_sig&&(cut3||cut0||cut1);
       bool bkg_shape  = cuta&&cutb&&cutc&&!cut_sig&&!K_2&&(cut0||cut1||cut3)&&!cut_k3pi;
       //bool k3pi_shape = cuta&&cutb&&cutc&&!cut_sig&&(cut0||cut1||cut3)&&cut_k3pi;
       bool k3pi_shape = cuta&&cutb&&cutc&&(cut0||cut1||cut3)&&cut_k3pi;
   ...
   TFile *f = new TFile("figure/sig.root","recreate");
   //TFile *f = new TFile("figure/offical_bkg.root","recreate");
   //TFile *f = new TFile("figure/k3pi_bkg.root","recreate");
   sig->Write();
   f->Close(); 
