...
for (int nmode=0; nmode<3;nmode++) {
   TChain t("tagD");
   //t.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sigMC/update/K3pi/red_truth/truth*.root");//K3pi
   //t.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/new/inclusiveMC/umiss2fit/reduce_truth/root/truth*.root");//inclusive mc~ other bkg
   t.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sigMC/update/BELLE/fit2/red_truth/truth.root");
...
/*
if(nmode==0)   bool k3pi_shape = cuta&&cutb&&cutc&&cut0&&cut_k3pi;//mode0
if(nmode==1)   bool k3pi_shape = cuta&&cutb&&cutc&&cut1&&cut_k3pi;
if(nmode==2)   bool k3pi_shape = cuta&&cutb&&cutc&&cut3&&cut_k3pi;//B
*/    if(sig_shape){ 
         Umissfit = Umissfit;
         mhad = mhad;  
         sig->Fill();  
    ...
    /*if(k3pi_shape){
      ...
      //TFile *f = new TFile(Form("dat/offical_bkg_%d.root",nmode),"recreate");
      //TFile *f = new TFile(Form("dat/k3pi_bkg_%d.root",nmode),"recreate");
      TFile *f = new TFile(Form("dat/sig_%d.root",nmode),"recreate");
