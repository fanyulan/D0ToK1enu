...
for (int nmode=3; nmode<4; nmode++) {
    ...
    TFile *f = TFile::Open(Form("root/et1_dh3_d0bar_mc.root",nmode));    //mc
    //TFile *f = TFile::Open(Form("root/et1_dh3_d0bar_data.root",nmode));//data requirement
   ...
   if(nmode==3)    RooRealVar  sigma1("sigma1", "", 0.,4e-3);
   if(nmode==3)    RooRealVar  sigma2("sigma2", "", 0.,2e-3);
...
*/
    while ((edm>1||status!=0||covQual!=3/*||chi2>3*/)&&times<1000){
     ...
     Double_t m_sigma1 = r1.Uniform(0,4e-3);
     Double_t m_sigma2 = r1.Uniform(0,2e-3);
  ...
  //plot on 
    ...
    rp1->SetMarkerSize(0.5);
     ...
     lt1.DrawLatex(0.22, 0.76,  "#chi^{2}_{ndof} =1.8");
    ...
    /************************************************end********************************************************/
    ofstream fit_data;
    fit_data.open(Form("./figure/mode3/mc_1_%d_d0bar_mc.dat",nmode), ios::app);
    //fit_data.open(Form("./figure/mode3/mc_1_%d_d0bar_data.dat",nmode), ios::app);
    ...
    */
     ...
     cvs->Print(Form("figure/mode3/mc_1_%d_d0bar_mc.eps",nmode),"recreate");    
     //cvs->Print(Form("figure/mode3/mc_1_%d_d0bar_data.eps",nmode),"recreate");    
     cout<<"chi2 "<<chi2<<endl;
     cout<<"eff "<<eff<<endl;
     }
}
