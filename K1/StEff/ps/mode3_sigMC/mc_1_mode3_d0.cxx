...
for (int nmode=3; nmode<4; nmode++) {
    ...
    TFile *f = TFile::Open(Form("root/et1_dh3_d0_mc.root",nmode)); //mc
    //TFile *f = TFile::Open(Form("root/et1_dh3_d0_data.root",nmode));//data 
   ...
   if(nmode==3)    RooRealVar  mean1( "mean1" , "", -3e-3,3e-3);
   if(nmode==3)    RooRealVar  mean2( "mean2" , "", -3e-3,3e-3);
   if(nmode==3)    RooRealVar  sigma1("sigma1", "", 0.,1e-3);
   if(nmode==3)    RooRealVar  sigma2("sigma2", "", 0.,4e-3);
    ...
    if(nmode==3)    RooRealVar Nsig("Nsig","", 0.95*sum_data, 0.0, sum_data);
    if(nmode==3)    RooRealVar Nbkg("Nbkg","", 0.05*sum_data, 0.0, sum_data);
...
*/
    while ((edm>1||status!=0||covQual!=3/*||chi2>3*/)&&times<500){
     Double_t m_mean1  = r1.Uniform(3e-3, 3e-3); 
     Double_t m_mean2  = r1.Uniform(3e-3, 3e-3);
     Double_t m_sigma1 = r1.Uniform(0, 1e-3);
     Double_t m_sigma2 = r1.Uniform(0, 4e-3);
     Double_t m_frac1  = r1.Uniform(0,1);     
     Double_t m_kappa  = r1.Uniform(-50.,30.);
     Double_t m_arg2   = r1.Uniform(0.,10.);
   ...
   /********************************************end of calculate the chisq***********************************************/
    ...
    rp1->SetMarkerSize(0.5);
     ...
     lt1.DrawLatex(0.22, 0.76,  "#chi^{2}_{ndof} = 1.9");
    ...
    double N_prod3 = 1027500;//1000000.0+27500.0;
    ...
    /************************************************end********************************************************/
    ofstream fit_data;
    fit_data.open(Form("./figure/mode3/mc_1_%d_d0_mc.dat",nmode),ios::app);
    //fit_data.open(Form("./figure/mode3/mc_1_%d_d0_data.dat",nmode),ios::app);
    ...
    fit_data<<"kappa: "<<kappa<<"  arg2: "<<arg2<<"\n";
    ...
    */
     ...
     cvs->Print(Form("figure/mode3/mc_1_%d_d0_mc.eps",nmode),"recreate");    
     //cvs->Print(Form("figure/mode3/mc_1_%d_d0_data.eps",nmode),"recreate");    
     //cvs->Print(Form("figure/mc_1_%d_d0_deltaE.eps",nmode),"recreate");    
     cout<<"chi2 "<<chi2<<endl;
     cout<<"eff "<<eff<<" err: "<<N_fit_err/N_prod3<<endl;
     }
}
