...
for (int nmode=0; nmode<1; nmode++) {
   ...
   if(nmode==3)    RooRealVar  sigma2("sigma2", "", 0.,1e-3);
    ...
    if(nmode==3) RooRealVar arg2  ("arg2", "", 0, 10.); 
...
*/
    while ((edm>1||status!=0||covQual!=3/*||chi2>3*/)&&times<1000){
     ...
     Double_t m_mean2  = r1.Uniform(mean2.getMin()+1e-4, mean2.getMax()-1e-4);
     Double_t m_sigma1 = r1.Uniform(sigma1.getMin()+2e-4,sigma1.getMax()-2e-4);
     Double_t m_sigma2 = r1.Uniform(sigma2.getMin()+2e-4,sigma2.getMax()-2e-4);
     Double_t m_frac1  = r1.Uniform(frac1.getMin()+1e-1,frac1.getMax()-1e-1);     
