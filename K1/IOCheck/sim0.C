...
using namespace RooFit;
    ...
    TChain ch("tagD");
    ch.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sig/ps/DDecay_root/D0_1.root");
    ch.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sig/ps/DDecay_root/D0bar_1.root");
    ch.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sig/ps/bkg_incl/bkg_D0D0bar/for_IO/D0D0bar_bkg0.root");
    ch.Add("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/sig/ps/bkg_incl/bkg_D0D0bar/for_IO/D0D0bar_bkg0_1.root");


    TTree* t0 = ch.CopyTree(normal&&m0&&!K_2);
    TTree* t1 = ch.CopyTree(normal&&m1&&!K_2);
    TTree* t3 = ch.CopyTree(normal&&m3&&!K_2);
  ...
  double Ntag[3];
  for(int i=0;i<3;i++){
    ifstream ntag("/besfs/groups/psipp/psippgroup/public/fangyl/cocktail/script/mBC/io/ps/figure/mc0.txt");
    ntag>>Ntag[0]>>Ntag[1]>>Ntag[2];
  }

  double nmis[3];
  for(int i=0;i<3;i++) {
    ifstream Nmis("misidentify103/mis0.txt");  
    Nmis>>nmis[0]>>nmis[1]>>nmis[2];
  }
    ...
    RooRealVar nbkg0("nbkg0", "", 10,1,30); //nbkg.setConstant(1);
    RooRealVar nbkg1("nbkg1", "", 40,1,80); //nbkg.setConstant(1);
    RooRealVar nbkg3("nbkg3", "", 20,1,50); //nbkg.setConstant(1);
    
    RooRealVar nbkg0_k3pi("nbkg0_k3pi", "", nmis[0]);//2.72 
    RooRealVar nbkg1_k3pi("nbkg1_k3pi", "", nmis[1]);//6.51
    RooRealVar nbkg3_k3pi("nbkg3_k3pi", "", nmis[2]);//4.13 
...
///////////////////////////////////////////////////////////////////modes///////////////////
    ...
    hmhadall->SetXTitle("M(K#pi#pi) [GeV]");
     ...
     c1->Print("figure/sim0_modes.eps");
    cv2->Print("figure/sim0.eps");
    ofstream out;
    out.open("figure/sim0.dat");
    ...
    out<<"total fit    : "<<Nsig0+Nsig1+Nsig3<<"+/-"<<Nsig1_err*(Nsig0+Nsig1+Nsig3)/Nsig1;
     ...
     ofstream out_1("figure/bf_new.txt", ios::app);
     out_1<<"sim0: "<<setprecision(9)<<bf_K1<<"\t"<<bf_K1_err<<"\t"<<"\n";
