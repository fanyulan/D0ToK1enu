...
for (int nmode=3; nmode<4;nmode++) {
    ...
    TFile *f = TFile::Open(Form("./root/datahist0_%d.root",nmode));
    ...
    double bins = 200; 
    TH1F* h2 = new TH1F("h2","",bins,1.8365,1.8865);
   ...
   if(nmode==0)    RooRealVar  sigma1("sigma1", "", 0.,1e-3);
   if(nmode==0)    RooRealVar  sigma2("sigma2", "", 0.,3e-3);

   if(nmode==1)    RooRealVar  mean1( "mean1" , "", -2e-4,2e-4);  
   if(nmode==1)    RooRealVar  mean2( "mean2" , "", -1e-3,1e-3);
   if(nmode==1)    RooRealVar  sigma1("sigma1", "", 0.,1e-2);
   if(nmode==1)    RooRealVar  sigma2("sigma2", "", 0.,1e-3);
   ...
   if(nmode==3)    RooRealVar  sigma2("sigma2", "", 0.,1e-3); 
    ...
    RooRealVar arg2  ("arg2", "", 0.3, 5.); 
    RooArgusBG bkg("bkg","",mBC,endpt,kappa,arg2);
    
    double sum_data = hdata->sumEntries();
   
    if(nmode==0)    RooRealVar Nsig("Nsig","", 0.9*sum_data, 0.0, sum_data);     
    if(nmode==0)    RooRealVar Nbkg("Nbkg","", 0.1*sum_data, 0.0, sum_data);    
...
*/
    while ((edm>1||status!=0||covQual!=3/*||chi2>3*/)&&times<1000){
     Double_t m_mean1  = r1.Uniform(mean1.getMin()+1e-4, mean1.getMax()-1e-4);
     Double_t m_mean2  = r1.Uniform(mean2.getMin()+1e-4, mean2.getMax()-1e-4);
     Double_t m_sigma1 = r1.Uniform(sigma1.getMin()+1e-4,sigma1.getMax()-1e-4);
     Double_t m_sigma2 = r1.Uniform(sigma2.getMin()+1e-4,sigma2.getMax()-1e-4);
...
*/
  ...
  /************************************************end********************************************************/
  ofstream fit_data;
  fit_data.open(Form("./figure/mc0_%d.dat",nmode));
...
if(!(nmode==0)) TArrow *ar2 = new TArrow(1.874, 0.5*h1->GetMaximum(),1.874,0.1*h1->GetMaximum() ,0.01,"|>");
    ...
    cvs->cd();
    cvs->SetSelected(cvs);
    cvs->Print(Form("figure/mc0_%d.eps",nmode),"recreate");
    cout<<"chi2 "<<chi2<<endl;
}
}
